...
class PlasterResultScreen extends StatelessWidget {
  ...
  String _area(double value) { final system = MeasurementPreferences.system.value ?? MeasurementSystem.metric; return '${_number(MeasurementPreferences.fromSquareMetres(value, system), 2)} ${system.areaUnit}'; }
  @override Widget build(BuildContext context) { final colors = Theme.of(context).colorScheme; return AppScaffold(title: 'Calculation Result', bodyBuilder: (context, padding) => ListView(padding: padding, children: [
    ...
    Card(color: colors.primary, child: Padding(padding: const EdgeInsets.all(18), child: Row(children: [Container(width: 46, height: 46, decoration: BoxDecoration(color: Colors.white.withValues(alpha: .16), borderRadius: BorderRadius.circular(14)), child: const Icon(Icons.format_paint_rounded, color: Colors.white)), const SizedBox(width: 14), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Total Area', style: TextStyle(color: Colors.white.withValues(alpha: .78))), Text(_area(result.area), style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w700))]))]))), const SizedBox(height: 12),
