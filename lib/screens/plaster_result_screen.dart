...
class PlasterResultScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final displayArea = MeasurementPreferences.fromSquareMetres(
      result.area,
      system,
    );
    final unitCost = EstimateCostCalculator.perUnit(
      result.totalCost,
      displayArea,
    );
    return AppScaffold(
      ...
      bodyBuilder: (context, padding) => ListView(
        padding: padding,
        children: [
          ResultHeader(label: 'Plaster Quantity', value: _area(result.area)),
          const SizedBox(height: 10),
          MaterialRequirementCard(
            materials: [
              MaterialRequirement(
                'Thickness',
                '${_number(result.thicknessMm)} mm',
                Icons.straighten_rounded,
              ),
              MaterialRequirement(
                'Cement',
                '${_number(result.cementBags)} bags',
                Icons.inventory_2_outlined,
              ),
              MaterialRequirement(
                'Sand',
                '${_number(result.sandM3)} m3',
          ...
          _section(context, 'Labour & Time', Icons.groups_outlined, [
            _row(
              context,
              'Crew',
              '${productivity.crew[LabourRole.mason]} Mason(s) + ${productivity.crew[LabourRole.helper]} Helper(s)',
            ),
            _row(
              context,
              'Working Days',
          ...
          CalculationDetails(
            children: [
              ...
              _row(
                context,
                'Mason mandays',
                _number(productivity.mandays[LabourRole.mason]!),
              ),
              _row(
                context,
                'Helper mandays',
              ...
              _row(context, 'Wet Volume', _number(result.wetVolume)),
              _row(context, 'Dry Volume', _number(result.dryVolume)),
              const Text(
                'Working days use the controlling role requirement for the selected crew.',
              ),
            ],
          ),
          const SizedBox(height: 10),
