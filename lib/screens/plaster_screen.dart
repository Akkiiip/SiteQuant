...
class _PlasterScreenState extends State<PlasterScreen> {
  ...
  double? _positive(TextEditingController c, String label) {
    final value = double.tryParse(c.text.trim());
    if (value == null || value <= 0) {
      _error('Enter a value greater than zero for $label.');
      return null;
    }
    return value;
  }
  ...
  void _calculate() {
    final length = _positive(_length, 'Length'),
        second = _positive(
          _second,
          _type == PlasterType.wall ? 'Wall Height' : 'Width',
        ),
        walls = _type == PlasterType.wall
            ? _positive(_walls, 'Number of Walls')
            : 1.0,
        thickness = _thickness == 'Custom'
            ? _positive(_customThickness, 'Thickness')
            : double.parse(_thickness),
        wastage = _positive(_wastage, 'Wastage');
    if (length == null ||
        second == null ||
        walls == null ||
        thickness == null ||
        wastage == null)
      return;
    if (_type == PlasterType.wall && walls != walls.roundToDouble()) {
      _error('Number of Walls must be a whole number.');
      return;
    }
    final system =
    ...
    final result = PlasterCalculator.calculate(
      area:
          MeasurementPreferences.toMetres(length, system) *
          MeasurementPreferences.toMetres(second, system) *
          walls,
    ...
    bodyBuilder: (context, padding) => SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Estimate plaster material quantities.',
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'Plaster Thickness',
                      floatingLabelBehavior: FloatingLabelBehavior.always,
                    ),
                    items: const ['12', '15', '20', 'Custom']
                        .map(
                          (value) => DropdownMenuItem(
                            value: value,
                            child: Text(
                              value == 'Custom' ? value : '$value mm',
                            ),
                          ),
                        )
                        .toList(),
